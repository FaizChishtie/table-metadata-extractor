...
class TableMetadata():
    ...
    @staticmethod
    def create_table_from_metadata(table_metadata, database_uri, num_rows, last_updated = "last_updated"):
        name = table_metadata.name
        schema = table_metadata.schema
        num_rows = num_rows
        database = str(database_uri)
        last_updated = last_updated # not tracked by psql
        columns = []
